...
function LeaveRequestManager() {
  ...
  return (
    <div className="w-[70%] mx-auto my-6">
      <h2 className="text-2xl font-semibold text-center mb-6">📋 Leave Requests (Manager)</h2>

      {leaveRequests.length === 0 ? (
        <p className="text-center text-gray-500">No leave requests found.</p>
      ) : (
        <div className="flex flex-col gap-4">
          {leaveRequests.map(({ employeeDetails, leaveDetails }) => (
            <div
              key={leaveDetails.leaveRequestId}
              className="border-l-4 border-indigo-500 rounded shadow bg-white overflow-hidden"
            >
              <div className="flex justify-between items-center px-5 py-3 bg-gray-100">
                ...
                <div className="flex flex-col items-end">
                  <span className="text-sm font-medium px-3 py-1 rounded-full bg-indigo-100 text-indigo-700 mb-1">
              ...
              <div className="px-5 py-4 space-y-3">
                ...
                {leaveDetails.approvalStatus.managerApproval === "Pending" && (
